...
public class TaskService {
    ...
    public List<Task> getTasks(String labelId, String priority, String status, String sortBy) {
        ...
        if (priority != null && !priority.isBlank() && labelId != null && !labelId.isBlank()) {
            tasks = tasks.stream().filter(task -> priority.equalsIgnoreCase(task.getPrioritas())).toList();
        }
        if (status != null && !status.isBlank() && (labelId != null && !labelId.isBlank() || priority != null && !priority.isBlank())) {
            tasks = tasks.stream().filter(task -> status.equalsIgnoreCase(task.getStatus())).toList();
        }
    ...
    public Task getTask(String id) {
        return taskRepository.findWithLabelsById(id).orElseThrow(() -> new RuntimeException("Task tidak ditemukan: " + id));
    }
    ...
    public Task addLabel(String taskId, String labelId) {
        ...
        Label label = labelRepository.findById(labelId).orElseThrow(() -> new RuntimeException("Label tidak ditemukan: " + labelId));
        task.addLabel(label);
        return taskRepository.save(task);
    }
    ...
    private List<Task> sortTasks(List<Task> tasks, String sortBy) {
        if (sortBy == null || sortBy.isBlank()) return tasks;
        return switch (sortBy.toLowerCase()) {
            case "deadline" -> tasks.stream().sorted(Comparator.comparing(Task::getDeadline, Comparator.nullsLast(Comparator.naturalOrder()))).toList();
            case "priority", "prioritas" -> tasks.stream().sorted(Comparator.comparingInt(this::priorityWeight)).toList();
            case "createdat", "created_at" -> tasks.stream().sorted(Comparator.comparing(Task::getCreatedAt, Comparator.nullsLast(Comparator.reverseOrder()))).toList();
            default -> tasks;
        };
    }

    private int priorityWeight(Task task) {
        if (task.getPrioritas() == null) return 99;
